  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (row[CONFIG.COLS.WB_ID - 1]) { // Если есть артикул WB
      products.push({
        ...
        minPrice: row[CONFIG.COLS.MIN_PRICE - 1] || 0, // Минимальная цена
...
function updateProductInSheet(product, updates) {
  ...
  if (updates.minPrice !== undefined) values[CONFIG.COLS.MIN_PRICE - 1] = updates.minPrice; // Минимальная цена
...
function loadProductsFromWB() {
  ...
  try {
    ...
    data.push(['Фото', 'Артикул WB', 'Артикул продавца', 'Название', 'Бренд', 'РРЦ', 'Модель удержания',
               ...
               'Цена с кошельком', '% кошелька', 'Минимальная цена', 'Загружаемая цена', 'Статус загрузки']);
