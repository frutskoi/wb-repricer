// Обновление кода Apps Script с новыми endpoint-ами WB API
...
// Готовый код с обновлёнными endpoint-ами
const UPDATED_CODE = `
/**
 ...
 */
function fetchWBApi(endpoint, method, apiKey, data = null, retries = 3) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (responseCode === 200 || responseCode === 201) {
        ...
        console.log(\`⏱️ Rate limit exceeded, пауза 2с...\`);
      ...
      if (attempt === retries) {
        console.error(\`❌ Ошибка после \${retries} попыток:\`, e.message);
        throw e;
      }
      console.log(\`⚠️ Попытка \${attempt} не удалась: \${e.message}, повтор через 1с...\`);
...
function fetchProducts() {
  ...
  for (const endpoint of endpoints) {
    ...
    try {
      ...
      if (nmIds.length === 0) {
        console.log('⚠️ В таблице нет товаров для загрузки из API');
        return [];
      }
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        try {
          ...
          if (cards && cards.length > 0) {
            ...
            console.warn(\`⚠️ В ответе нет карточек\`);
...
async function updateAppsScript() {
  ...
  try {
    ...
    const backupPath = '/home/clawd/.openclaw/workspace/wb-repricer/Code.gs.backup';
    fs.writeFileSync(backupPath, jsFile.source || '');
    console.log(`💾 Сохранён бэкап: ${backupPath}`);

    // Обновляем код
    jsFile.source = UPDATED_CODE;
