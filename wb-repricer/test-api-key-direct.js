// Прямая проверка ключей WB API
const https = require('https');
const fs = require('fs');
const path = require('path');

// Читаем ключ из настроек таблицы
const settingsPath = path.join(__dirname, 'check-settings-output.json');

// Прямо из предыдущего вывода
const API_KEY = 'eyJhbGciOiJFUzI1NiIsImtpZCI6IjIwMjYwMzAydjEiLCJ0eXAiOiJKV1QifQ..r_r3LLDAFpl0E_iQCs7ipDjuQbMVusdls5HgNEnAJqJSm1tUUTKhmKcKza6MvfPM6n4bTlUKStHx8Ra0l3WHtg';

console.log('🔍 Прямая проверка WB API с ключом из настроек\n');

// Пробуем правильный эндпоинт (suppliers-api)
function testSuppliersAPI() {
  console.log('📡 Пробую suppliers-api.wildberries.ru...\n');

  const options = {
    hostname: 'suppliers-api.wildberries.ru',
    port: 443,
    path: '/content/v1/cards/cursor/list',
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${API_KEY}`,
      'Content-Type': 'application/json'
    }
  };

  const req = https.request(options, (res) => {
    let data = '';

    console.log(`📡 Статус ответа: ${res.statusCode}`);
    console.log(`📋 Заголовки:`);
    console.log(JSON.stringify(res.headers, null, 2));
    console.log();

    res.on('data', (chunk) => {
      data += chunk;
    });

    res.on('end', () => {
      try {
        const jsonData = JSON.parse(data);
        console.log('📊 Ответ от WB API:\n');
        console.log(JSON.stringify(jsonData, null, 2));

        if (res.statusCode === 200) {
          console.log('\n✅ Ключ РАБОТАЕТ на suppliers-api!');
          if (jsonData.data && jsonData.data.cards) {
            console.log(`📦 Получено карточек: ${jsonData.data.cards.length}`);
          }
        } else if (res.statusCode === 401 || res.statusCode === 403) {
          console.log('\n❌ Ключ НЕ РАБОТАЕТ:');
          console.log('   - 401: Не авторизован (ключ неверный или истёк)');
          console.log('   - 403: Нет прав доступа');
        } else {
          console.log(`\n⚠️ Статус: ${res.statusCode}`);
        }
      } catch (e) {
        console.log('❌ Ошибка парсинга JSON:', e.message);
        console.log('📄 Сырой ответ:');
        console.log(data);
      }
    });
  });

  req.on('error', (error) => {
    console.error('❌ Ошибка запроса:', error.message);
  });

  req.write(JSON.stringify({ limit: 10 })); // Запрашиваем только 10 товаров
  req.end();
}

testSuppliersAPI();
